...
        }; 
        ...
        $scope.showNoticeProduct = function () {
			console.log(JSON.stringify($scope.accTypeForNoticeProduct)+','+$scope.AccountType);
			for (var i=0; i<$scope.accTypeForNoticeProduct.length; i++){
            console.log('for loop:'+$scope.accTypeForNoticeProduct[i].Name);
				if($scope.AccountType == $scope.accTypeForNoticeProduct[i].Name){
					return true;
				}
			}
			return false;	
