        ...
        */
        ...
        var getUrl = function (obj) {
            ...
            return base + '&ent=Case';
                ...
                var ltngbase = baseUrl +'/lightning/o/Case/new?recordTypeId='+$scope.caseId+'&nooverride=true&count=1';
                ltngbase = ltngbase+'&defaultFieldValues=';
                if (obj.internalStaff) {
                    // var staffFound, accountIdStaff;
                    
                    if (accountIdStaff.length) {
                        ltngbase = ltngbase + 'AccountId=' + accountIdStaff;
                    }
                    
                    if (staffFound.length) {
                        ltngbase = ltngbase + ',ContactId=' + staffFound;
                    }
                } else {
                    //alert('questionCtrl accountId : '+accountId+' staffNotFound : '+staffNotFound+' staffFound : '+staffFound);
                    //alert('staffNotFoundlgtn : '+staffNotFoundlgtn);
                    if (accountId.length) {
                        ltngbase = ltngbase + 'AccountId=' + accountId;
                    }
                    if (staffNotFoundlgtn.length) {
                        ltngbase = ltngbase + ',ContactId=' + staffNotFoundlgtn;
                    }
                    /*if (!staffNotFound.length && staffFound.length) {
                        ltngbase = ltngbase + ',ContactId=' + staffFound;
                    }*/
                }
                //alert('ltngbase : '+ltngbase);
                console.log ('ltngbase' + ltngbase);
                return ltngbase ;
            }
