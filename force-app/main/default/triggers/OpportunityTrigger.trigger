...
trigger OpportunityTrigger on Opportunity (after insert, after update, before insert, before update) 
{
    ...
    // Added by Deepak to bypass trigger for Attachment referecece updates on Opportunities records.
    if(Trigger_Helper.runOpportunityTrigger==false)
    {
        return;
    }
    // End
    ...
    */
    ...
    if(trigger.isBefore)
    {
        if(trigger.isUpdate)
        {
            ...
            for (Opportunity opp : trigger.new)
            {
                ...
                if(opp.RecordTypeId == liabilitiesRecordId && opp.stagename != Trigger.oldMap.get(opp.Id).stagename && 'Account declined'.equalsIgnoreCase(opp.StageName)){
                    opp.Printing_Status__c = 'To be printed';
                    opp.To_Be_Printed_Date__c = system.today().adddays(1);
                }
    ...
    if (trigger.isAfter){
        if (trigger.isUpdate){
            ...
            if(oppIds.isempty()) {
                OpportunityLetterGeneratorHelper.generateOppLetters(oppIds);
            }
        }
        if (trigger.isInsert){
